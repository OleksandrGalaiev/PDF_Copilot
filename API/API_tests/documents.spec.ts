...
test.describe('Documents API', { tag: '@api' }, () => {
  test(
    'GET /documents/recent — returns 200 with valid schema',
    { tag: '@documents' },
    async ({ api, config }) => {
  ...
  test(
    'GET /documents/recent — each document has required fields',
    { tag: '@documents' },
    async ({ api, config }) => {
      const data = await api.url(config.PDF_API_PROD).path('/documents/recent').GET_Request(200);
  ...
  test(
    'GET /user/account — returns 200 with valid schema',
    { tag: '@documents' },
    async ({ api, config }) => {
