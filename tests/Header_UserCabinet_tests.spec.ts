...
test.describe('Session restore and logout', { tag: '@sanity' }, () => {
  test(
    'Restored session is valid and user can sign out via header menu',
    { tag: '@loginLogout' },
    async ({ app, baseURL }) => {
...
test.describe('User cabinet', { tag: '@sanity' }, () => {
  ...
  test(
    'Account settings popup displays correct email',
    { tag: '@userCabinet' },
    async ({ app }) => {
